...
This device now can be seen on Provision >> Inventory

![x](/static/2024-11-23-sda/43.png)

<br>

With that process, NADs also automatically created on the ISE side
...
On Activities >> Tasks, we can see the provisioning progress
...
Now we can see that both devices are in the fabric, but they still dont have role in it
...
And now this fabric is up and running, where client can access the network
