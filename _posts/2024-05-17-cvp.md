---
title: CVP & Call Studio Scripting
...
Here we create an ICM Script to with "user.microapp.ToVXML[0]" set to "application=helenaapp", and then take the "Call.user.microapp.caller_input" as a parameter to decide wether a music should be played or to forward the call to an agent
...
<Context>
    <Resource 
		url="jdbc:sqlserver://localhost:1433;databaseName=dummy;username=sa;password=thepassword;encrypt=true;trustServerCertificate=true"
...
After that, we create a Digit Input element to receive the 4 digit ID number from the caller

![x](/static/2024-05-17-cvp/30.png)

<br>

Then use the Database Element to create a query to the set up database using the input ID as a parameter
...
And when the call connects, we can see the data is fetched into Variable 4
