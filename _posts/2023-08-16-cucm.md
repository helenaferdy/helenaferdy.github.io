...
All non bootable iso has a bootable bootloader information stored as a file called isolinux.bin, extract the file
...
Then select Yes when asked if this node is the Node
...
Now the Web UI should be accessible on 198.18.0.15
...
Next create an End User used to login on User Management >> End User
...
After that create endpoint phone device, which in this case we'll user jabber. <br>
...
Now we try making a call from 1001 to 9002, this call should be received by the extension 1002
