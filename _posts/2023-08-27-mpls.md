...
We have 5 Routers running MPLS where router xe1 and xe5 will become the PE (Provider Edge), meaning it'll be the connecting router to the Customer's router (CE or Customer Edge), which will be the cEdge0 and cEdge1. <br>
...
Now if we run the command "show ip bgp vpnv4 all", we can see OSPF routes being detected by BGP
...
Now we need the CE to see each other's OSPF routes through BGP, so lets redistribure BGP on OSPF. <br>
Run this command
...
And now, if we do an end-to-end tracing from CE0 to CE1 (Customer2) and vice versa, we should see everything the traffic being routed though MPLS network
...
And if we do end-to-end trace, we can see it also being routed successfully through MPLS network
