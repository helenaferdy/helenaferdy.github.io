...
The attack successfully prevented because the signature of the exploit was already in our IPS Protections Database, allowing the firewall to detect it
...
After the victim downloads and runs the file, it will try to initiate a connection to the listener
...
We can also customize how the blocked pages shown to end users by selecting the template here
...
Without Anti-virus protection, users can easily download this infected file from the inside host with no problems
...
Now if we try to download the text file again, we'll get blocked error
